...
class CostEvent(BaseModel):
    ...
    source: Literal["budgets", "anomaly"] = Field(..., description="Source of the cost event")
...
class TimeWindow(BaseModel):
    ...
    days: list[str] = Field(..., description="Days of week: mon, tue, wed, thu, fri, sat, sun")
...
class PolicyExceptions(BaseModel):
    ...
    accounts: list[str] | None = Field(default=None, description="Account IDs to always exempt")
...
class PolicyMatch(BaseModel):
    ...
    max_amount_usd: float | None = Field(default=None, gt=0, description="Optional maximum cost")
    services: list[str] | None = Field(
        default=None, description="AWS service names (e.g., 'Amazon EC2')"
    )
    regions: list[str] | None = Field(default=None, description="AWS regions (e.g., 'us-east-1')")
...
class PolicyScope(BaseModel):
    ...
    principals: list[Principal] = Field(..., min_length=1, description="IAM principals to restrict")
    regions: list[str] | None = Field(default=None, description="AWS regions")


class PolicyAction(BaseModel):
    """Action to execute when policy matches."""

    type: Literal["attach_deny_policy", "notify_only"] = Field(..., description="Action type")
    ...
    @classmethod
    def validate_deny_actions(cls, v: list[str] | None, info) -> list[str] | None:
...
class NotificationSettings(BaseModel):
    ...
    channel_hint: str | None = Field(default=None, description="Slack channel name (informational)")
...
class GuardrailPolicy(BaseModel):
    ...
    actions: list[PolicyAction] = Field(..., min_length=1, description="Actions to execute")
    notify: NotificationSettings = Field(..., description="Notification settings")
    exceptions: PolicyExceptions | None = Field(default=None, description="Exception rules")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "actions": [{"type": "attach_deny_policy", "deny": ["ec2:RunInstances"]}],
                "notify": {
...
class ActionPlan(BaseModel):
    ...
    matched_policy_id: str | None = Field(default=None, description="ID of matched policy")
    mode: Literal["dry_run", "manual", "auto"] | None = Field(
    ...
    ttl_minutes: int | None = Field(default=None, description="TTL for auto-rollback")
...
class ActionExecution(BaseModel):
    ...
    executed_at: datetime | None = Field(default=None, description="When the action was executed")
    executed_by: str = Field(..., description="Who executed (email or 'system:auto')")
    ...
    ttl_expires_at: datetime | None = Field(
        default=None, description="When auto-rollback should occur"
    )
    rolled_back_at: datetime | None = Field(default=None, description="When rollback occurred")
...
class NotificationPayload(BaseModel):
    ...
    notification_type: Literal["dry_run", "approval_request", "execution_confirmed", "rollback"] = (
